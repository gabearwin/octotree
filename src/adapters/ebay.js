const EBAY_GH_RESERVED_USER_NAMES = [
  'settings',
  'orgs',
  'organizations',
  'site',
...
const EBAY_GH_RESERVED_REPO_NAMES = ['followers', 'following', 'repositories'];
const EBAY_GH_404_SEL = '#parallax_wrapper';
...
const EBAY_GH_PJAX_CONTAINER_SEL =
  '#js-repo-pjax-container, div[itemtype="http://schema.org/SoftwareSourceCode"] main, [data-pjax-container]';

const EBAY_GH_CONTAINERS = '.container, .container-lg, .container-responsive';
const EBAY_GH_HEADER = '.js-header-wrapper > header';
const EBAY_GH_RAW_CONTENT = 'body > pre';
const EBAY_GH_MAX_HUGE_REPOS_SIZE = 50;
const EBAY_GH_HIDDEN_RESPONSIVE_CLASS = '.d-none';
const EBAY_GH_RESPONSIVE_BREAKPOINT = 1010;

class Ebay extends PjaxAdapter {
  constructor(store) {
    super(store);
  }
  ...
  // @override
  init($sidebar) {
    const pjaxContainer = $(EBAY_GH_PJAX_CONTAINER_SEL)[0];
  ...
  updateLayout(sidebarPinned, sidebarVisible, sidebarWidth) {
    const SPACING = 10;
    const $header = $(EBAY_GH_HEADER);
    const $containers =
      $('html').width() <= EBAY_GH_RESPONSIVE_BREAKPOINT
        ? $(EBAY_GH_CONTAINERS).not(EBAY_GH_HIDDEN_RESPONSIVE_CLASS)
        : $(EBAY_GH_CONTAINERS);
  ...
  getRepoFromPath(currentRepo, token, cb) {
    // 404 page, skip
    if ($(EBAY_GH_404_SEL).length) {
      return cb();
    }

    // Skip raw page
    if ($(EBAY_GH_RAW_CONTENT).length) {
      return cb();
    }
    ...
    if (~EBAY_GH_RESERVED_USER_NAMES.indexOf(username) || ~EBAY_GH_RESERVED_REPO_NAMES.indexOf(reponame)) {
      return cb();
    }
  ...
  // @override
  selectFile(path) {
    super.selectFile(path, {pjaxContainerSel: EBAY_GH_PJAX_CONTAINER_SEL});
  }
      ...
      .done((data) => {
        if (path && path.indexOf('/git/trees') === 0 && data.truncated) {
          ...
          if (!hugeRepos[repo]) {
            ...
            if (repos.length >= EBAY_GH_MAX_HUGE_REPOS_SIZE) {
